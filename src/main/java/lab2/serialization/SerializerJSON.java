...
public class SerializerJSON implements Serializer{
    ...
    public void listToFile(List<Ticket> tickets, String fileName){
        try {
            objectMapper.writeValue(new File(fileName), tickets);
        }
        catch (IOException exc) {
            throw new RuntimeException(exc);
        }
    ...
    public static void main(String args[]) {
        ...
        serializer.toFile(ticket, "ticketJSON");
        ...
        tickets.add(ticket);
        serializer.listToFile(tickets, "ticketsJSON");

        ticket = serializer.fromFile("ticketJSON");
        System.out.println(ticket);
        tickets = serializer.listFromFile("ticketsJSON");
